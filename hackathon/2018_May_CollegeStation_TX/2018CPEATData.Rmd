...
```{r Bear}
dataFile<-'~/TestingStuff/Bear.csv'

allData <- read_csv(file=dataFile, skip = 1)

metaData <- allData[,1:2] %>%  #read first 2 columns as meta data
  ...
  spread(key=site_name,value=Bear)
sampleData <- allData[,5:14] %>% #identify columns for sample reads
  gather(key = 'header', value = 'value', -depth, -peat_type) %>% #convert to long format exclude                                                                       columns that are charters not numarics
  filter(!is.na(value)) %>% #reverts missing values                                
  mutate(value=as.numeric(value)) #make sure values are numaric
...
sampleData <- allData[,17:23] %>% #identify columns for age
  ...
  filter(!is.na(value))
  bind_rows(sampleData) #combine with previous tables
